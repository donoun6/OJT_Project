...
# union all
-- 총 합계 계산
SELECT SUM(selling.quantity)    AS quantity,
       SUM(selling.total_price) AS total_price
FROM selling
WHERE 1 = 1
...
GROUP BY product.name, product.sell_price, product.image
...
WHERE selling.check_selling = TRUE
  AND orders.check_orders = TRUE
GROUP BY month, selling.selling_id, selling.quantity, selling.total_price,
         selling.check_selling, selling.register_date, orders.orders_id, orders.check_orders
...
call AddSellingAndClearCartTest('2023-11-10');
